...
// ChunkTexts creates evidence chunks from a list of texts
// Generic function that works with any string content (tool results, documents, etc.)
func ChunkTexts(texts []string, windowSize int, idPrefix string) []EvidenceChunk {
	if windowSize < 1 {
		windowSize = 2
	}

	chunks := make([]EvidenceChunk, 0, len(texts)*3)
	...
	for textIdx, text := range texts {
		...
		textChunks := chunkSingleText(text, textIdx, windowSize, idPrefix)
...
// chunkSingleText splits a single text into overlapping sentence windows
func chunkSingleText(text string, textIdx int, windowSize int, idPrefix string) []EvidenceChunk {
	...
	chunks := make([]EvidenceChunk, 0, len(sentences))

	if len(sentences) == 1 {
		chunkID := createChunkID(idPrefix, textIdx, 0)
		chunks = append(chunks, EvidenceChunk{
			ID:         chunkID,
			Text:       sentences[0],
			Metadata:   map[string]string{"source_index": strconv.Itoa(textIdx)},
			StartIndex: 0,
	...
	for i := 0; i < len(sentences); i++ {
		...
		startIndex := findSentenceStart(text, windowSentences[0])
		endIndex := findSentenceEnd(text, windowSentences[len(windowSentences)-1])
		chunkID := createChunkID(idPrefix, textIdx, i)
...
// createChunkID generates a unique chunk identifier
func createChunkID(prefix string, textIdx, chunkIdx int) string {
	if prefix == "" {
		prefix = "chunk"
	}
	return prefix + "-" + strconv.Itoa(textIdx) + "-" + strconv.Itoa(chunkIdx)
}
