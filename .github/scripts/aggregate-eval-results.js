        ...
        try {
            ...
            const commentPath = path.join(fullPath, 'comment.md');
                ...
                if (fs.existsSync(providerNamePath) && fs.existsSync(commentPath)) {
                    ...
                    let commentBody = fs.readFileSync(commentPath, 'utf8').trim();
                    ...
                    if (commentBody) {
                        ...
                        commentBody = commentBody.replace(/^## 🤖 .*$/m, '').trim();

                        // Remove the individual footer (we'll add one unified footer at the end)
                        commentBody = commentBody.replace(/\*This comment was automatically generated by the eval CI pipeline\.\*/g, '').trim();
