...
For AI models with vision capabilities, attach an image file to your message when chatting with an Agent to ask questions about the image or request analysis. The Agent responds based on the visual content. Vision capabilities are enabled by default for supported bots. Ensure the underlying AI model supports vision features.
